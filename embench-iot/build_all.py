...
def compile_file(f_root, srcdir, bindir, suffix='.c'):
    ...
    log.debug('### Compiling support')
    log.debug('### f_root: {f_root}'.format(f_root=f_root))
    log.debug('### srcdir: {srcdir}'.format(srcdir=srcdir))
    log.debug('### bindir: {bindir}'.format(bindir=bindir))
    ...
    log.debug('Succeeded command was:')
    log.debug(arglist_to_str(arglist))
    return succeeded
