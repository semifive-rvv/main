...
def output_stats(geomean, geosd, georange, count, bm_type, opt_comma):
    ...
    if count > 0:
        if gp['absolute']:
            if gp['output_format'] == output_format.JSON:
                ...
                geomean_op = '{gm:17,}'.format(gm=round(geomean))
                geosd_op = '{gs:17.2f}'.format(gs=geosd)

            georange_op = '{gr:17.2f}'.format(gr=georange)
        else:
            if gp['output_format'] == output_format.JSON:
    ...
    if gp['output_format'] == output_format.JSON:
        ...
        log.info('---------                -----')
        log.info('Geometric mean   {gmo:18}'.format(gmo=geomean_op))
        log.info('Geometric SD     {gso:18}'.format(gso=geosd_op))
        log.info('Geometric range  {gro:18}'.format(gro=georange_op))
