...
def decode_results(stdout_str, stderr_str):
    ...
    # Match "bbl loader"
    rcstr = re.search('^bbl loader', stdout_str, re.S)
    ...
    # Match "CYCLES=rc"
    log.debug(stdout_str)
    #rcstr = re.search('^CYCLES=(\d+)', stdout_str, re.S)
    rcstr = re.search('CYCLES=(\d+)', stdout_str, re.S)
    log.debug("### rcstr.grout(1)")
    log.debug(rcstr.group(1))

    if not rcstr:
        log.debug('Warning: Failed to find return code')
        return 0.0
    else:
        log.debug('Success: Find # of cycles')
        return int(rcstr.group(1))
